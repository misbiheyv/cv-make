...
import {INVALID_RESUME_DATA, VALID_RESUME_DATA} from "../fixtures/test-data";
...
describe("Browser Pool Integration", () => {
    ...
    describe("PDF generation", () => {
        ...
        it("handles concurrent requests successfully", async () => {
            const requests = Array.from({length: 5}, () => generatePdf(VALID_RESUME_DATA));
    ...
    describe("pool lifecycle", () => {
        ...
        it("returns all browsers to pool after requests complete", async () => {
            ...
            const requests = Array.from({length: 3}, () => generatePdf(VALID_RESUME_DATA));
            await Promise.all(requests);
